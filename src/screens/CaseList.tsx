    ...
    <div className="min-h-full">
      <header className="border-b border-[var(--border-default)] bg-[var(--bg-surface)] px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          <ThemeToggle />
          <label className="text-sm text-slate-600 dark:text-slate-400 flex items-center gap-1 mr-3">
            <input
              type="checkbox"
            ...
            {rows.map((r) => (
              <li key={r.id} className="flex items-stretch gap-2">
                <Link
                  ...
                  className="flex-1 block rounded-md border border-[var(--border-default)] bg-[var(--bg-surface)] px-4 py-3 hover:border-slate-400"
                >
